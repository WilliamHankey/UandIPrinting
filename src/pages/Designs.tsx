...
const Designs = () => {
  ...
  const { designs, categories, loading: isLoading, error } = useDesigns();
  ...
  const filteredDesigns = React.useMemo(() => {
    if (!designs) return [];
    let result = designs;
    if (filter !== 'All Designs') {
      result = result.filter(design => design.category === filter);
    }
    if (searchQuery.trim()) {
          ...
          <div className="flex flex-col md:flex-row gap-4 justify-between items-center">
            <div className="flex items-center gap-2 overflow-x-auto pb-2 w-full md:w-auto">
              {categoryList.map((cat) => (
                ...
                >
                  {cat === 'All Designs' && <Filter className="h-4 w-4 mr-2" />}
                  {cat}
                </Button>
              ))}
            </div>
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                {displayedDesigns.length === 0 && (
                  <div className="col-span-full text-center py-16">
                    <p className="text-gray-500 mb-4">No designs found{filter !== 'All Designs' ? ` in "${filter}"` : ''}{searchQuery ? ` matching "${searchQuery}"` : ''}.</p>
                    <Button variant="outline" onClick={() => { setFilter('All Designs'); setSearchQuery(''); setSearchParams({}); }}>
                      Clear Filters
                    </Button>
                  </div>
                )}
